...
Function fndecargs(sv ptrcontent,sd ptrsize,sd sz,sd ptr_stackoffset,sd parses)
	...
	if vartype>=(vnumbers)
		...
		if vartype==(valuesinnernumber)
			...
			setcall err xfile_add_char_if((Xfile_argtype_long))
		else
			if b==(TRUE)
			...
			if vartype==(integersnumber)
				setcall err xfile_add_char_if((Xfile_argtype_long_int))
			else
			#was vstringsnumber
				setcall err xfile_add_char_if((Xfile_argtype_long_byte))
			endelse
		endelse
	elseif vartype==(charnumber)
		set datasize (bsz)
		setcall err xfile_add_char_if((Xfile_argtype_byte))
	...
	elseif vartype==(integersnumber)
		setcall err xfile_add_char_if((Xfile_argtype_int))
	else
	#stringsnumber
		setcall err xfile_add_char_if((Xfile_argtype_long_byte))
	endelse
